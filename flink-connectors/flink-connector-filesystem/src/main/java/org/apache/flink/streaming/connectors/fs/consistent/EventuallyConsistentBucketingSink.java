	...
	private void initFileSystem() throws IOException {
		...
		if (fsConfig != null) {
			...
			for (String key : fsConfig.keySet()) {
				hadoopConf.setString(key, fsConfig.getString(key, null));
			}
		}
		remoteFileSystem = BucketingSink.createHadoopFileSystem(new Path(basePath), hadoopConf);
	}
		...
		private void initFileSystem(Configuration consistentFsConfig, String consistentFsDir) throws IOException {
			...
			if (consistentFsConfig != null) {
				...
				for (String key : consistentFsConfig.keySet()) {
					hadoopConf.setString(key, consistentFsConfig.getString(key, null));
				}
			}

			consistentFsPath = new Path(consistentFsDir, "flink-ecs-dir/" + Integer.toString(subtaskIndex));
			consistentFileSystem = BucketingSink.createHadoopFileSystem(consistentFsPath, hadoopConf);
